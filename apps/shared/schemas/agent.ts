...
export const agentInitialMessageSchema = z.object({
  chatId: z.string().uuid('Invalid chat ID'),
  userMessage: z.string().min(1, 'Message is required').max(50000, 'Message too long'),
})

export const agentContinueMessageSchema = z.object({
  chatId: z.string().uuid('Invalid chat ID'),
  toolResults: z.array(toolResultSchema).min(1, 'At least one tool result required'),
  previousContent: z.array(contentBlockSchema).min(1, 'Previous content required'),
})
