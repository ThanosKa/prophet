...
export async function POST(req: Request) {
  try {
    ...
    if (!rateLimitResult.success) {
      logger.warn({ userId, remaining: rateLimitResult.remaining }, 'Rate limit exceeded for agent chat')
    ...
    if (chat.userId !== userId) {
      return NextResponse.json(error('Forbidden', 'FORBIDDEN'), { status: 403 })
    }
    ...
    if (user.creditsRemaining < 10) {
      logger.warn({ userId, creditsRemaining: user.creditsRemaining }, 'Insufficient credits for agent chat')
      return NextResponse.json(
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
            ...
            if (event.type === 'message_start') {
              ...
              if (currentToolUse) {
                ...
                try {
                  ...
                  logger.warn({ toolUseId: currentToolUse.id, input: currentToolUse.input }, 'Failed to parse tool input')
                }
          ...
          logger.error({ error: err instanceof Error ? err.message : String(err), userId, chatId }, 'Agent streaming error')
    ...
    logger.error({ error: err instanceof Error ? err.message : String(err) }, 'Agent chat endpoint error')
    return NextResponse.json(
